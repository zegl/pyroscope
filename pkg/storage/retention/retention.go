...
type Retention struct {
	...
	dimensions    *badger.DB
	dicts         *badger.DB
	trees         *badger.DB
	treesCache    *cache.Cache
	main          *badger.DB
	labels        core.LabelsStore
	// exemplars core.ExemplarsStore
...
func (s *Retention) EnforceRetentionPolicy(ctx context.Context, rp *segment.RetentionPolicy) {
...
func (s *Retention) deleteSegmentData(ctx context.Context, k *segment.Key, rp *segment.RetentionPolicy) error {
	sk := k.SegmentKey()
	cached, ok := s.segmentsCache.Lookup(sk)
	if !ok {
		return nil
	}
	...
	if deleted {
		return s.core.Delete(context.TODO(), &types.DeleteInput{Key: k})
	}
...
func (s *Retention) reclaimSegmentSpace(k *segment.Key, size int64) error {
	...
	if len(rp.Levels) > 0 {
		if cached, ok := s.segmentsCache.Lookup(k.SegmentKey()); ok {
			if ok, err = cached.(*segment.Segment).DeleteNodesBefore(rp); ok {
				err = s.core.Delete(context.TODO(), &types.DeleteInput{Key: k})
			}
		}
	}
...
func (s *Retention) flushTreeBatch(batch *badger.WriteBatch) (*badger.WriteBatch, error) {
	if err := batch.Flush(); err != nil {
		return batch, err
	}
	select {
...
func (s *Retention) iterateOverAllSegments(cb func(*segment.Key) error) error {
	nameKey := "__name__"

	var dimensions []*dimension.Dimension
	s.labels.GetValues(nameKey, func(v string) bool {
		if d, ok := s.core.LookupAppDimension(v); ok {
