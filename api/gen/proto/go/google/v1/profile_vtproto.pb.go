...
func (m *Profile) CloneVT() *Profile {
	...
	r := &Profile{
		...
		PeriodType:        m.PeriodType.CloneVT(),
		Period:            m.Period,
...
func (m *Profile) ResetVT() {
	m.SampleType = m.SampleType[:cap(m.SampleType)]
	for i := range m.SampleType {
		if m.SampleType[i] != nil {
			m.SampleType[i].Reset()
		}
	}
	f0 := m.SampleType[:0]
	m.Sample = m.Sample[:cap(m.Sample)]
	...
	f1 := m.Sample[:0]
	m.Mapping = m.Mapping[:cap(m.Mapping)]
	...
	f2 := m.Mapping[:0]
	m.Location = m.Location[:cap(m.Location)]
	for i := range m.Location {
		if m.Location[i] != nil {
			m.Location[i].ResetVT()
		}
	}
	f3 := m.Location[:0]
	m.Function = m.Function[:cap(m.Function)]
	for i := range m.Function {
		if m.Function[i] != nil {
			m.Function[i].ResetVT()
		}
	}
	f4 := m.Function[:0]
	f5 := m.StringTable[:0]
