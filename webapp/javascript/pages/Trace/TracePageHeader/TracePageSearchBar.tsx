...
export function TracePageSearchBarFn(
  props: TracePageSearchBarProps & { forwardedRef: React.Ref<Input> }
) {
  const {
    clearSearch,
    focusUiFindMatches,
  ...
  const count = textFilter ? (
    <span className="TracePageSearchBar--count">{resultCount}</span>
  ) : null;

  const btnClass = cx('TracePageSearchBar--btn', {
    'is-disabled': !textFilter,
  });
...
export default React.forwardRef(
  (props: TracePageSearchBarProps, ref: React.Ref<Input>) => (
