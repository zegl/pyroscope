...
const fullActions = createActions<TActionTypes>({
  ...
  [actionTypes.DETAIL_LOG_ITEM_TOGGLE]: (spanID: string, logItem: Log) => ({
    logItem,
    spanID,
  }),
  ...
  [actionTypes.SET_TRACE]: (trace: Trace, uiFind: string | TNil) => ({
    trace,
    uiFind,
  }),
});
...
function calculateFocusedFindRowStates(
  ...
  if (matchedSpanIds && matchedSpanIds.size) {
    matchedSpanIds.forEach((spanID) => {
      ...
      spanAncestorIds(span).forEach((ancestorID) =>
        childrenHiddenIDs.delete(ancestorID)
      );
    });
...
function focusUiFindMatches(
  ...
  { uiFind, trace, allowHide }: TTraceUiFindValue
) {
...
function detailLogItemToggle(
  state: TTraceTimeline,
  { spanID, logItem }: TSpanIdLogValue
) {
  const old = state.detailStates.get(spanID);
  if (!old) {
    return state;
...
function addHoverIndentGuideId(
  state: TTraceTimeline,
  { spanID }: TSpanIdValue
) {
...
function removeHoverIndentGuideId(
  state: TTraceTimeline,
  { spanID }: TSpanIdValue
) {
  const newHoverIndentGuideIds = new Set(state.hoverIndentGuideIds);
...
export default handleActions(
  {
    [actionTypes.ADD_HOVER_INDENT_GUIDE_ID]: guardReducer(
    ...
    [actionTypes.DETAIL_REFERENCES_TOGGLE]: guardReducer(
    ...
    [actionTypes.REMOVE_HOVER_INDENT_GUIDE_ID]: guardReducer(
