...
): ShamefulAny<{ followsFrom: boolean }>[] {
  return edges.map((e) => {
    let hasChildOf = true;
    if (typeof e.to === 'number') {
      ...
      hasChildOf = node.members.some(
        (m) =>
          m.span.references &&
          m.span.references.some((r) => r.refType === 'CHILD_OF')
      );
    }
...
function getChildOfDrange(parentID: string, trace: Trace) {
  ...
  getChildOfSpans(parentID, trace).forEach((s) => {
    ...
    childrenDrange.add(
      s.startTime,
      s.startTime + (s.duration <= 0 ? 0 : s.duration - 1)
    );
  });
  return childrenDrange;
}

export function calculateTraceDag(
  trace: Trace
): TraceDag<TSumSpan & TDenseSpanMembers> {
  ...
  baseDag.nodesMap.forEach((node) => {
    ...
    const numErrors = node.members.reduce(
      (p, m) => (p + isError(m.span.tags) ? 1 : 0),
      0
    );
    const childDurationsDRange = node.members.reduce((p, m) => {
      ...
      const cdr = new DRange(
        m.span.startTime,
        m.span.startTime + m.span.duration
      ).intersect(getChildOfDrange(m.span.spanID, trace));
      return p + cdr.length;
    }, 0);
