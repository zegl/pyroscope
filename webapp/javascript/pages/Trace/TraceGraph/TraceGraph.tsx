...
const { classNameIsSmall, scaleOpacity, scaleStrokeOpacity } =
...
const HELP_CONTENT = (
  <div className="TraceGraph--help-content">
    ...
    <div>
      (*) <b>Self time</b> is the total time spent in a span when it was not
      waiting on children. For example, a 10ms span with two 4ms non-overlapping
      children would have <b>self-time = 10ms - 2 * 4ms = 2ms</b>.
    </div>
  </div>
);
...
export default class TraceGraph extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    if (!ev) {
      return (
        <h1 className="u-mt-vast u-tx-muted ub-tx-center">No trace found</h1>
      );
    }

    const wrapperClassName = cx('TraceGraph--graphWrapper', {
      'is-uiFind-mode': uiFind,
    });

    return (
      <div
        className={wrapperClassName}
        style={{ paddingTop: headerHeight + 47 }}
      >
        <Digraph<TDagPlexusVertex<TSumSpan & TDenseSpanMembers>>
          ...
          layers={[
            ...
            },
            {
              ...
              measurable: true,
              renderNode: cacheAs(
                `trace-graph/nodes/render/${mode}`,
                getNodeRenderer(mode)
              ),
            },
          ]}
