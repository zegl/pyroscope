...
export default class OpNode extends React.PureComponent<Props> {
  render() {
    ...
    const popoverContent = (
      <div className="OpNode--popoverContent">{table}</div>
    );
...
export function getNodeRenderer(mode: string) {
  return function drawNode(
    vertex: TDagPlexusVertex<TSumSpan & TDenseSpanMembers>
  ) {
    return <OpNode {...vertex.data} mode={mode} />;
  };
}

export function getNodeFindEmphasisRenderer(
  uiFindVertexKeys: Set<string> | null | undefined
) {
  return function renderFindEmphasis(
    lv: TLayoutVertex<TDagPlexusVertex<TSumSpan & TDenseSpanMembers>>
  ) {
...
export function renderNodeVectorBorder(
  lv: TLayoutVertex<TDagPlexusVertex<TSumSpan>>
) {
  return (
    <rect
