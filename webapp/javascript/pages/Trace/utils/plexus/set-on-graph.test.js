...
describe('Set on graph utils', () => {
  describe('setOnNodesContainer', () => {
    function getComputedWidth(k) {
      const { style } = setOnNodesContainer({
        zoomTransform: k != undefined ? { k } : undefined,
  ...
  describe('setOnEdgesContainer', () => {
    ...
    it('calculates style object with opacity off of zoomTransform.k', () => {
      expect(
        setOnEdgesContainer({ zoomTransform: { k: 0.0 } }).style.opacity
      ).toBe(0.1);
      expect(
        setOnEdgesContainer({ zoomTransform: { k: 0.3 } }).style.opacity
      ).toBe(0.37);
      expect(
        setOnEdgesContainer({ zoomTransform: { k: 0.5 } }).style.opacity
      ).toBe(0.55);
      expect(
        setOnEdgesContainer({ zoomTransform: { k: 0.7 } }).style.opacity
      ).toBe(0.73);
      expect(
        setOnEdgesContainer({ zoomTransform: { k: 1.0 } }).style.opacity
      ).toBe(1);
    });
  });
