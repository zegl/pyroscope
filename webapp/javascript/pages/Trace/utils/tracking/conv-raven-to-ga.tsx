...
function convBreadcrumbs(crumbs: ICrumb[]) {
  ...
  let c = '';
  let ci = -1;
  const compacted = joiner.reduce(
    (accum: string[], value: string, j: number): string[] => {
      if (value === c) {
        return accum;
      }
      if (c) {
...
function getLabel(
  ...
  const header = [message, page, duration, git, '']
    .filter((v) => v != null)
    .join('\n');
...
export default function convRavenToGa({ data }: RavenTransportOptions) {
  ...
  let action = [message, tags && tags.git, url, '', stack]
    .filter((v) => v != null)
    .join('\n');
