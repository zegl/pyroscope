...
const GA: IWebAnalyticsFunc = (
  ...
  const isErrorsEnabled =
    isDebugMode || Boolean(_get(config, 'tracking.trackErrors'));
  ...
  const trackRavenError = (ravenData: RavenTransportOptions) => {
    const { message, category, action, label, value } =
      convRavenToGa(ravenData);
    trackError(message);
  ...
  const init = () => {
    ...
    if (cookiesToDimensions !== undefined) {
      (
        cookiesToDimensions as unknown as Array<{
          cookie: string;
          dimension: string;
        }>
      ).forEach(
        ({ cookie, dimension }: { cookie: string; dimension: string }) => {
          ...
          // eslint-disable-next-line no-console
            ...
            console.warn(
              `${cookie} not present in cookies, could not set dimension: ${dimension}`
            );
        }
      );
    }
    if (isErrorsEnabled) {
      ...
      window.onunhandledrejection = function trackRejectedPromise(
