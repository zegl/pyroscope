  ...
  Object.values(traces).forEach(({ data }) => {
    if (data) {
      ...
      tree.paths((pathIds: string[]) => {
        const paths = pathIds.reduce(
          (reducedSpans: Span[], id: string): Span[] => {
            if (id === TREE_ROOT_ID) {
              return reducedSpans;
            }
            ...
            if (!span)
              throw new Error(
                `Ancestor spanID ${id} not found in trace ${traceID}`
              );
            if (reducedSpans.length === 0) {
              reducedSpans.push(span);
            } else if (
              reducedSpans[reducedSpans.length - 1].processID !==
                span.processID ||
              isKindServer(span)
            ) {
              reducedSpans.push(span);
        ...
        const path: TDdgPayloadEntry[] = paths.map(
          ({ processID, operationName: operation }) => ({
        ...
        if (
          path.some(
            ({ service, operation }) =>
              ...
              (!focalOperation || operation === focalOperation)
          )
        ) {
          const pathKey = path
            .map((value) => `${value.operation}:${value.service}`)
            .join('/');
