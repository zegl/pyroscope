...
const stringifyEntry = ({ service, operation }: TDdgPayloadEntry) =>
  `${service}\v${operation}`;
...
function transformDdgData(
  ...
  const paths = dependencies
    ...
    .map(({ path: payloadPath, attributes }) => {
      ...
      hashArg.push(
        pathCompareValues.get(payloadPath) ||
          payloadPath.map(stringifyEntry).join()
      );
      ...
      path.members = payloadPath.map(
        ({ operation: operationName, service: serviceName }, i) => {
