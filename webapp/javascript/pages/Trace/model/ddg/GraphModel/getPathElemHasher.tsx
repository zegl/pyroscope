...
function fmtElemShowOp(pe: PathElem) {
  return pe.distance === 0
    ? FOCAL_KEY
    : `${pe.operation.service.name}\t${pe.operation.name}`;
}
...
function getExtVsIntHasher(elemToStr: TPathElemToStr) {
  return (pe: PathElem) =>
    `${getPpeHasher(elemToStr)(pe)}${pe.isExternal ? '; is-external' : ''}`;
}
...
export default function getPathElemHasher(this: GraphModel): TPathElemToStr {
  ...
  switch (this.density) {
    case EDdgDensity.MostConcise: {
      return (pe: PathElem) => {
        ...
        if (pe.operation.service.name === focalElem.operation.service.name) {
          ...
          // never be incomplete.
          const focalOps = new Set(
            Array.from(focalElems).map(({ operation }) => operation.name)
          );
    ...
    case EDdgDensity.UpstreamVsDownstream: {
      return (pe: PathElem) =>
        pe.distance
          ? `${elemToStr(pe)}; direction=${Math.sign(pe.distance)}`
          : FOCAL_KEY;
    }
    case EDdgDensity.OnePerLevel: {
      return (pe: PathElem) =>
        pe.distance ? `${elemToStr(pe)}; distance=${pe.distance}` : FOCAL_KEY;
    }
    ...
    default: {
      throw new Error(
        `Density: ${
          this.density
        } has not been implemented, try one of these: ${JSON.stringify(
          EDdgDensity,
          null,
          2
        )}`
