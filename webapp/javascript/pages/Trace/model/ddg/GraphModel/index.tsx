...
export default class GraphModel {
  readonly getDerivedViewModifiers = memoize(3)(
    getDerivedViewModifiers.bind(this)
  );
  ...
  constructor({
    ...
    this.visIdxToPathElem.forEach((pathElem) => {
      ...
      if (connectedElem) {
        ...
        if (!connectedVertex) {
          throw new Error(
            `Non-focal pathElem cannot be connected to graph. PathElem: ${pathElem}`
          );
        }
  ...
  private getGeneration = (
    ...
    elems.forEach(({ focalSideNeighbor, memberIdx, memberOf }) => {
      ...
      if (generationMember && generationMember !== focalSideNeighbor)
        rv.push(generationMember);
    });
    return rv;
  };
  ...
  public getGenerationVisibility = (
    ...
    const visibleGeneration = generation.filter(({ visibilityIdx }) =>
      visibleIndices.has(visibilityIdx)
    );

    if (visibleGeneration.length === generation.length)
      return ECheckedStatus.Full;
  ...
  public getVisibleIndices(visEncoding?: string): Set<number> {
    return new Set(
      this.getVisiblePathElems(visEncoding).map(
        ({ visibilityIdx }) => visibilityIdx
      )
    );
  }

  public getVisible: (visEncoding?: string) => {
    edges: TEdge[];
    vertices: TDdgVertex[];
  } = memoize(10)(
    (visEncoding?: string): { edges: TEdge[]; vertices: TDdgVertex[] } => {
      ...
      if (this.visIdxToPathElem.length) {
        ...
        const visibleFocalElems = this.getVertexVisiblePathElems(
          FOCAL_KEY,
          visEncoding
        );
  ...
  private static getUiFindMatches(
    vertices: TDdgVertex[],
    uiFind?: string
  ): Set<string> {
    ...
    for (let i = 0; i < vertices.length; i++) {
      ...
      const ops =
        operation &&
        (Array.isArray(operation) ? operation : [operation]).map((op) =>
          op.toLowerCase()
        );
      for (let j = 0; j < uiFindArr.length; j++) {
        if (
          svc.includes(uiFindArr[j]) ||
          (ops && ops.some((op) => op.includes(uiFindArr[j])))
        ) {
  ...
  public getHiddenUiFindMatches: (
    uiFind?: string,
    visEncoding?: string
  ) => Set<string> = memoize(10)(
    (uiFind?: string, visEncoding?: string): Set<string> => {
      ...
      if (this.visIdxToPathElem.length) {
        ...
        const visibleFocalElems = new Set(
          this.getVertexVisiblePathElems(FOCAL_KEY, visEncoding)
        );
  ...
  public getVisibleUiFindMatches: (
    uiFind?: string,
    visEncoding?: string
  ) => Set<string> = memoize(10)(
    (uiFind?: string, visEncoding?: string): Set<string> => {
  ...
  public getVisWithoutVertex(
    vertexKey: string,
    visEncoding?: string
  ): string | undefined {
    const elems = this.getVertexVisiblePathElems(vertexKey, visEncoding);
    if (elems && elems.length)
      return this.getVisWithoutElems(elems, visEncoding);
    return undefined;
  }
  ...
  public getVisWithUpdatedGeneration(
    ...
    const generationElems = this.getGeneration(
      vertexKey,
      direction,
      visEncoding
    );
    const currCheckedStatus = this.getGenerationVisibility(
      vertexKey,
      direction,
      visEncoding
    );
