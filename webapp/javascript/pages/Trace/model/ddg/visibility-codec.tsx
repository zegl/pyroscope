 ...
 */
export const decode: (encoded: string) => number[] = memoize(10)(
  (encoded: string): number[] => {
    ...
    encoded.split(DELIMITER).forEach((tupleStr) => {
      ...
      for (
        let j = 0;
        partialAsNumber >= 1 << j && j < VISIBILITY_BUCKET_SIZE;
        j += 1
      ) {
...
export const encode = (decoded: number[]): string => {
  ...
  decoded.forEach((visIdx) => {
    const { csvIdx, visibilityValue } =
      convertAbsoluteIdxToRelativeValues(visIdx);
  ...
  return condensed
    .map(([partial, count]) => {
      ...
      const suffix =
        count !== 1 ? `${COUNT_INDICATOR}${count.toString(36)}` : '';
...
export const encodeDistance = ({
  ...
  if (Math.sign(distance) === -1 * Math.sign(direction)) {
    throw new Error(
      `Distance (${distance}) and direction (${direction}) cannot have opposite signs`
    );
  }
  ...
  if (prevVisEncoding) {
    nextVisible = decode(prevVisEncoding).filter(
      (idx) =>
        visIdxToPathElem[idx] &&
        Math.sign(visIdxToPathElem[idx].distance) !== direction
    );
  } else {
    nextVisible = [
