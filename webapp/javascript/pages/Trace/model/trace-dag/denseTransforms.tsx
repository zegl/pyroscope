...
function fixLeafService(denseSpan: TDenseSpan, map: Map<string, TDenseSpan>) {
  ...
  if (
    parentOp.indexOf(peerSvc) === 0 &&
    parentOp.slice(-operation.length) === operation
  ) {
...
function skipClient(denseSpan: TDenseSpan, map: Map<string, TDenseSpan>) {
  ...
  if (
    ...
    parent.skipToChild =
      parent.operation.indexOf(service) === 0 ||
      parentPeerSvc.indexOf(service) === 0;
  }
}
...
function skipAnnotationSpans(
  denseSpan: TDenseSpan,
  map: Map<string, TDenseSpan>
) {
...
function skipClientSpans(denseSpan: TDenseSpan, map: Map<string, TDenseSpan>) {
  ...
  denseSpan.skipToChild =
    child.tags[tagKeys.SPAN_KIND] === 'client' &&
    parent.span.processID === span.processID;
}

export default function denseTransforms(
  denseSpan: TDenseSpan,
  map: Map<string, TDenseSpan>
) {
