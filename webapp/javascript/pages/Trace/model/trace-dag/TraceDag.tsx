...
export default class TraceDag<TData extends { [k: string]: unknown } = {}> {
  static newFromTrace(
    trace: Trace,
    idFactory: TIdFactory = ancestralPathParentOrLeaf
  ) {
    const dag: TraceDag<TDenseSpanMembers> = new TraceDag();
    const { denseSpansMap, rootIDs } = new DenseTrace(trace);

    function addDenseSpan(
      denseSpan: TDenseSpan | undefined,
      ...
      children.forEach((childId) =>
        addDenseSpan(denseSpansMap.get(childId), id)
      );
    }
  ...
  static diff(a: TraceDag<TDenseSpanMembers>, b: TraceDag<TDenseSpanMembers>) {
    ...
    function makeDiffNode(id: NodeID) {
      ...
      const parentNodeID =
        (nodeA && nodeA.parentID) || (nodeB && nodeB.parentID) || null;
      const members = [
        ...(nodeA ? nodeA.members : []),
        ...(nodeB ? nodeB.members : []),
      ];
      dag.addNode(id, parentNodeID, {
        ...
        operation:
          (nodeA && nodeA.operation) ||
          (nodeB && nodeB.operation) ||
          '__UNSET__',
        service:
          (nodeA && nodeA.service) || (nodeB && nodeB.service) || '__UNSET__',
      });
    }
