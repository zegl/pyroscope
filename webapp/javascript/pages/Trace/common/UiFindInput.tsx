    ...
    const suffix = (
      <>
        {allowClear && inputValue && inputValue.length && (
          <Icon type="close" onClick={this.clearUiFind} />
        )}
        {inputProps.suffix}
      </>
    );
...
export function extractUiFindFromState(
  state: ReduxState
): TExtractUiFindFromStateReturn {
  const { uiFind: uiFindFromUrl } = queryString.parse(
    state?.router?.location?.search
  );
  const uiFind = Array.isArray(uiFindFromUrl)
    ? uiFindFromUrl.join(' ')
    : uiFindFromUrl;
  return { uiFind };
}
...
export default withRouter(
  connect(extractUiFindFromState)(UnconnectedUiFindInput)
);
