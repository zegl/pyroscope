...
export default class FilteredList extends React.PureComponent<TProps, TState> {
  ...
  private getFilteredCheckbox(filtered: string[]) {
    ...
    const valueSet =
      typeof value === 'string' || !value ? new Set([value]) : value;
    let checkedCount = 0;
      ...
      <Tooltip title={title}>
        <Checkbox
          ...
          onChange={({ target: { checked: newCheckedState } }) => {
            if (newCheckedState)
              addValues(filtered.filter((f) => !valueSet.has(f)));
  ...
  private onKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    switch (event.key) {
      ...
      case EKey.ArrowDown: {
        ...
        if (stFocused == null) {
          focusedIndex =
            event.key === EKey.ArrowDown ? visibleStartIndex : visibleStopIndex;
          ...
          focusedIndex =
            i > -1 ? i % filteredOptions.length : filteredOptions.length + i;
          this.setState({ focusedIndex });
        }
        const listInstance = this.vlistRef.current;
        if (
          listInstance &&
          (focusedIndex < visibleStartIndex + 1 ||
            focusedIndex > visibleStopIndex - 1)
        ) {
      ...
      case EKey.Enter: {
        ...
        if (stFocused !== null) this.setValue(filteredOptions[stFocused]);
          ...
          this.setValue(filteredOptions[0]);
        break;
      }
      default: // no-op
    }
  ...
  private onListItemsRendered = _debounce(
    (viewInfo: ListOnItemsRenderedProps) => {
