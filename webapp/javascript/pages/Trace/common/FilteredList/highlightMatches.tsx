...
function toHighlights(
  isHighlightGroup: TIsHighlightGroupFn,
...
const highlightEvenGroups = toHighlights.bind(
  null,
  (index: number) => index % 2 === 0
);
const highlightFirstGroup = toHighlights.bind(
  null,
...
class MatchHighlighter {
  ...
  constructor(readonly query: string) {
    ...
    const startsWith = `^(${tx})`;
    const wordStartsWith = `(\\b${letters
      .map(letterClass)
      .join('')}|${initialCap})`;
    const acronymLetters = letters
      .map((letter) => wordBreak(letter))
      .join(ANY_LAZY);
    const constains = `(${tx})`;
    ...
    this.matchers = [
      ...
      // /(.*?)(\b[eE][gG]|Eg)(.*)/
      [
        new RegExp(`${ANY_LAZY}${wordStartsWith}${ANY_GREEDY}`),
        highlightEvenGroups,
      ],
      ...
      // /(.*?)(\be|E)(.*?)(\bg|G)(.*)/
      [
        new RegExp(`${ANY_LAZY}${acronymLetters}${ANY_GREEDY}`),
        highlightEvenGroups,
      ],
      ...
      // /(.*?)(eg)(.*)/i
      [
        new RegExp(`${ANY_LAZY}${constains}${ANY_GREEDY}`, 'i'),
        highlightEvenGroups,
      ],
      ...
      // /(.*?)(e)(.*?)(g)(.*)/i
      [
        new RegExp(`${ANY_LAZY}${anyLetters}${ANY_GREEDY}`, 'i'),
        highlightEvenGroups,
      ],
    ];
  }
