...
function hasSpans(
  example: TExample | TExampleWithSpans
): example is TExampleWithSpans {
  return Boolean(example.spanIDs && example.spanIDs.length);
}

function getGetUrlArg(examples: TExample[]): {
  ...
  examples.forEach((example: TExample) => {
    if (hasSpans(example))
      spanLinks[example.traceID] = example.spanIDs.join(' ');
...
export default class ExamplesLink extends React.PureComponent<TProps> {
  render() {
    ...
    return (
      <a
        href={getUrl(getGetUrlArg(examples))}
        target="_blank"
