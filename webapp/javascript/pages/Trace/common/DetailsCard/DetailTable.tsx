...
export const _makeFilterDropdown =
  (dataIndex: string, options: Set<string>) =>
...
export const _sort = (dataIndex: string) => (a: TRow, b: TRow) => {
  ...
  else if (typeof aData === 'object' && typeof aData.value === 'string')
    aValue = aData.value;
  else aValue = aData;
  ...
  else if (typeof bData === 'object' && typeof bData.value === 'string')
    bValue = bData.value;
  else bValue = bData;
...
export const _makeColumns = ({
  ...
  defs.map((def: TColumnDef | string) => {
    ...
    return {
      ...
      title,
      filterDropdown:
        Boolean(options.size) && _makeFilterDropdown(dataIndex, options),
